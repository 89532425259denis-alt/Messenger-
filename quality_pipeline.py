...
# Ссылка считается корректной и как [N], и как [N, с. X]. Бот НЕ знает
# реальных страниц, поэтому требовать [N, с. X] везде = заставлять модель
# выдумывать страницы. [N] честная и не нарушает ГОСТ.
...
def validate_block_content(text: str, *, min_chars: int = 400,
    ...
    if require_citations and not has_valid_citation:
        reasons.append("нет ни одной ссылки формата [N] или [N, с. X]")
    ...
    # Аудит типичных ошибок генератора (обрывки предложений, склеенные
    # ссылки, оборванные сноски). Детекция, а не автоправка: блок уходит
    # на перегенерацию, если проблема найдена.
    try:
        import humanizer
        for problem in humanizer.audit_text(t):
