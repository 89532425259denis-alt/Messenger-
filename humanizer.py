...
_AI_MARKERS = (
    ...
    + [
        ...
        # Паттерны, найденные при аудите реферата (2026-08-30): генератор
        # без второго прохода пишет их на каждом шагу.
        "не только",
        "при этом",
        "колоссальн",
        ...
        "необходимо подчеркнуть",
        "важно подчеркнуть",
        "подобная пространственная организация",
...
# ──────────────────────────────────────────────────────────────────────
#  8. Аудит типичных ошибок генерации (по итогам разбора реферата)
# ──────────────────────────────────────────────────────────────────────

# Фрагменты, открывающие предложение, после которых в реферате шёл обрыв
# без сказуемого («Находящийся в непосредственной близости от дельты
# Селенги.»). Это деепричастный/причастный оборот, оторванный от главного
# предложения.
_FRAGMENT_OPENERS = re.compile(
    r"(?i)(?:^|(?<=[.!?])\s+|(?<=\])\s+|(?<=[«»\"])\s+)"
    r"(?:находящ\w*|расположенн\w*|представляющ\w*|относящ\w*|"
    r"принадлежащ\w*|характеризующ\w*|являющ\w*|используем\w*|"
    r"рассматриваем\w*|связанн\w*|обусловленн\w*|определяем\w*|"
    r"обеспечивающ\w*|составляющ\w*|включающ\w*|занимающ\w*|"
    r"располагающ\w*)\s+"
    r"(?=[а-яёА-ЯЁ\w\s]{4,}[.!?]?\s*$)"
)

# Ссылки, склеенные с текстом или друг с другом («[9, с. 24–25] с [1, с. 13]»,
# «[6, с [1, с. 13]. 32]», «с [1, с. 13]» — артефакт, когда модель дописывает
# «с» перед уже готовой ссылкой).
_MALFORMED_CITE = re.compile(
    r"(?i)(?:с|со|и)\s*\[\d+\s*,\s*[сСcC]\.\s*\d+\]"
    r"|\[\s*\d+\s*,\s*[сСcC]\.\s*\["
    r"|\[\s*\d+\s*,\s*[сСcC]\.\s*\d+\]\s*с\s*\["
)

# «Таких как ...» / «Также как ...» в начале предложения — обрывок
# перечисления, оставшийся без главного предложения.
_SENTENCE_OPEN_SUCH_AS = re.compile(r"(?i)(?:^|(?<=[.!?])\s+)таких\s+как\b")


def audit_text(text: str) -> list[str]:
    """Возвращает список проблем, релевантных типичным ошибкам генератора.

    Используется в валидаторах (quality_pipeline.validate_block_content и
    качественном шлюзе), чтобы отправить блок на перегенерацию ДО того, как
    ошибка попадает в готовый DOCX. Всё это — детекция, а не автоправка:
    согласование по роду/числу/падежу регулярками не чиним (см. принцип
    «неграмотный текст хуже шаблонного»).
    """
    if not text:
        return []
    out = []
    if _MALFORMED_CITE.search(text):
        out.append("ссылки склеены с текстом или между собой (например, «с [1, с. 13]» или «[6, с [1")
    if _SENTENCE_OPEN_SUCH_AS.search(text):
        out.append("предложение начинается с обрывка «Таких как ...» без главного предложения")
    frags = list(_FRAGMENT_OPENERS.finditer(text))
    if frags:
        out.append(f"обрывок предложения с причастием без сказуемого (например, «{frags[0].group(0).strip()[:40]}...»)")
    # Откровенно оборванная ссылка: [N, с. без цифры в конце абзаца/текста.
    if re.search(r"\[\s*\d+\s*,\s*[сСcC]\.\s*$", text):
        out.append("оборванная ссылка [N, с. без номера страницы")
    return out


def fix_glued_citation_prefix(text: str) -> str:
    """Убирает букву «с», случайно приклеенную перед ссылкой.

    Реферат: «...составляет около 636 километров [9, с. 24–25] с [1, с. 13].»
    Модель писала «...с [1, с. 13]» — «с» здесь лишняя.
    """
    if not text:
        return text
    # Убираем отдельно стоящее «с» / «со» / «и» непосредственно перед [N, с. X]
    # только когда перед ним уже есть законченная часть предложения.
    text = re.sub(r"(?i)\b(?:с|со|и)\s+(?=\[\d+\s*,\s*[сСcC]\.\s*\d+\])", "", text)
    # Убираем «с [N]» после другой ссылки.
    text = re.sub(r"(?i)(\])(?:\.|,)?\s*с\s+(?=\[\d+\])", r"\1 ", text)
    text = re.sub(r"\[\s*\d+\s*,\s*[сСcC]\.\s*\[", "[", text)
    return text
...
def humanize(text: str, *, aggressive: bool = True) -> str:
    ...
    out = fix_glued_citation_prefix(out)
    if aggressive:
        out = vary_rhythm(out)
