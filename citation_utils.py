    ...
    for para_idx, para in enumerate(paragraphs):
        ...
        if best_score >= 2:
            if not re.search(r"\[\d+\s*,\s*[сСcC]\.\s*\d+\]", para):
                # Страница не известна — ставим [N] без страницы. Выдумывать
                # номер страницы запрещено.
                if para.endswith("."):
...
def repair_broken_citations(text: str) -> str:
    """Чинит оборванные ссылки вида [1, с.] или [1,] в [N].

    СТРОГО без выдуманных номеров страниц."""
    if not text:
        return ""
