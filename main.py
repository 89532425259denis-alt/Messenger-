...
4. ★ v2.2: подглавы генерируются отдельными промптами (нет пустых глав).
   ...
   ГОСТ-сноски [N] или [N, с. 45] — в каждом абзаце, страница только реально известная.
...
ЭТАП 7. ПРОВЕРКА — ссылки [N] или [N, с. X], без выдуманных страниц
    ...
    for para_idx, para in enumerate(paragraphs):
        ...
        if best_score >= 2:
            if not re.search(r"\[\d+\s*,\s*[сСcC]\.\s*\d+\]", para):
                # Номер страницы не известен: ставим [N] без страницы.
                # Выдумывать страницу запрещено — это источник фактов, а не
                # «украшение» ссылки.
                if para.endswith("."):
    ...
    for key, text in parts.items():
        ...
        if bare_citations:
            ...
            def _fix_bare(m: re.Match) -> str:
                n = m.group(1)
                # Страницу берём ТОЛЬКО из того, что уже есть в тексте.
                # Если реальной страницы нет — оставляем [N] без страницы:
                # выдумывать номер страницы запрещено.
                if n in page_map:
...
def build_prompts(
    ...
    if doc_type == "article":
        ...
        return {
            ...
            "lit_review": strict_prompt(
                ...
                f"Ссылки оформляй как [N]. Номер страницы [N, с. X] указывай ТОЛЬКО если она реально "
                f"известна из источника; выдумывать страницы запрещено.",
                lit_review_chars,
                writing_style, doc_type,
            ),
    ...
    for i, j, ch_title, sub_title in all_subs:
        ...
        prompts[key] = strict_prompt(
            ...
            f"Используй 1–2 ссылки на источники в формате [N] или [N, с. X]. "
            f"Номер страницы указывай ТОЛЬКО когда он реально известен из источника; "
            f"выдумывать страницы ЗАПРЕЩЕНО — лучше [N], чем выдуманная страница. "
...
async def generate_text_blocks(
    ...
    ctx_mgr = ContextManager(topic=topic, doc_type=doc_type, subject=subject)
    ...
    if verified_bib:
        style_sys += (
            ...
            "СТРОГО ЗАПРЕЩЕНО придумывать номера страниц: если реальная страница из источника "
            "неизвестна, ставь ссылку в виде [N] без страницы — это допустимо по ГОСТ. "
    ...
    if doc_type == "esse":
        ...
        style_sys += (
            ...
            "Используй сноски [N] в каждом абзаце; номер страницы [N, с. X] — только если он реально известен. "
            "Терминология должна быть строгой, выверенной и соответствовать теме."
        )
    else:
        style_sys += (
            "Это АКАДЕМИ��ЕСКАЯ РАБОТА. "
            "В каждом абзаце используй 1–3 ГОСТ-сноски вида [N] (или [N, с. X], если номер "
            "страницы реально известен). Номера источников: 1, 2, 3... из списка литературы. "
            "Выдумывать номера страниц запрещено. "
        )
        if writing_style == "smart":
            style_sys += (
    ...
    style_sys += (
        ...
        "предложения. Ссылка оформляется как `[N]` (или `[N, с. P]`, если "
        "номер страницы реально известен; Cyrillic `с.`, не Latin `c.`). "
        "ВЫДУМЫВАТЬ номера страниц ЗАПРЕЩЕНО — неизвестная страница "
        "означает ссылку `[N]`. Н��КОГДА не оставляй `[N, с.` без номера. "
        "Не вставляй ссылку там, где не знаешь источник. "
        ...
        "❌ НЕПРАВИЛЬНО: «...осадочными породами [1, с.» — оборванная ссылка\n"
        "✅ ПРАВИЛЬНО: «...осадочными породами [1]» — если страница неизвестна\n"
        "✅ ПРАВИЛЬНО: «...осадочными породами [1, с. 45]» — страница известна\n\n"
        "Запомни: ссылка всегда заканчивается на ]. Страницу после «с.» пиши "
        "только если реально знаешь её.\n\n"
...
def _strip_duplicate_heading_prefix(text: str, heading: str) -> str:
    ...
    #    «1.1 Название. Далее идёт текст...» или «Название Далее текст...» —
    #    срезаем только префикс-заголовок, не трогая сам текст.
    heading_words = re.escape(_heading_compare_key(heading))
    if heading_words:
        # Регулярка по исходному тексту: (необязательный номер) + заголовок
        # + необязательная пунктуация-разделитель. Номер делаем НЕОБЯЗАТЕЛЬНЫМ:
        # модель часто пишет повтор заголовка в начале абзаца БЕЗ номера
        # («Координатные границы ... озера Озеро Байкал расположено...»),
        # когда DOCX уже вынес номер в отдельный заголовок.
...
async def generate_and_send(
    ...
    async with gen_priority_gate(pay_mode, _uid):
        ...
        try:
            ...
            if not _structure_approved and chapter_titles:
                try:
            ...
            # ── НОВОЕ: глубокий рерайт и проверка уникальности для антиплагиата ──
            plagiarism_caption_line = ""
            try:
                plag_api_key = cfg("PLAGIARISM_API_KEY", "")
                plag_api_url = cfg("PLAGIARISM_API_URL", "")
                if plag_api_key:
                    from plagiarism_checker import PlagiarismChecker
                    checker = PlagiarismChecker(api_key=plag_api_key)
                    full_text = "\n\n".join(v for k, v in parts.items() if k != "literature")
                    text, score = await checker.ensure_uniqueness(full_text, target=80.0)
                    plagiarism_caption_line = f"\n📊 <b>Уникальность:</b> {score:.1f}%"
            except Exception as e:
                print(f"[PLAGIARISM] Ошибка проверки уникальности: {e}")
